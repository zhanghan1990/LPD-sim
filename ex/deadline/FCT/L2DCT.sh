#!/bin/bash
## This is the LPD simulation result
path=$1
tcl_script="$path/searchl2dct";  
destdir=L2DCT
rm -rf $destdir
rm -r *.tr
rm -r *.Ex
...
top_dir="/home/zhanghan/ns-pfabric"
 ...
 do
  ...
  for((m=0; m<${#topology_x[*]}; m++))
    do
	  ...
	  cp flow.tr LPD-$curdir_-flow.tr
          mv flow.tr $curdir_
          mv $curdir_ $destdir
          mv LPD-$curdir_-flow.tr $destdir
       done
    done
 done
