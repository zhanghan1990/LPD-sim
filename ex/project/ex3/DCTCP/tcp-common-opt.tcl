...
#Use "setup" if possible in such cases.
    ...
    $self instvar deadline ;#deadline of the flow pair
...
TCP_pair instproc start { nr_bytes } {
    ...
    ## for each connection we set deadline here
    set   rng   [new RNG]
    $rng   seed   0
    ...
    $r3    set     max_       30000
    set    number [$r3 value]
    $tcps  set deadline_  10000000
    set deadline 10000000
    if {$nr_bytes <=1000000} {
      $tcps  set deadline_  [expr $number]
      set deadline [expr $number]
    }
...
# 
    ...
    set flow_deadline_init [$apair($pid) set deadline]
    set flow_deadline [expr $flow_deadline_init/1000000]
	...
	puts $logfile "$tmp_pkts $fldur $rttimes $flow_deadline"
	#puts $logfile "flow_stats: [$ns now] gid $group_id pid $pid fid $fin_fid bytes $bytes fldur $fldur actfl $actfl bps $bps deadline $flow_deadline"
	flush stdout
    }
