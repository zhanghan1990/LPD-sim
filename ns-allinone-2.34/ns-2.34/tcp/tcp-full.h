...
protected:
	...
	int nodelay_;       // disable sender-side Nagle?double threshold_min_;
	...
	// zhanghan for l2dct, wmin,wmax,thredshold_min, threshold_max
	...
	//zhanghan for LPD-s
	double S_MAX_; // flow size max in KB
	double D_MAX_; // deadline  max in us
	...
	*/
    ...
    int flow_size_; // Number of the flow to send
	...
	virtual void newack(Packet* pkt);	// process an ACK
...
class LLTcpAgent : public SackFullTcpAgent {
	...
	virtual int foutput(int seqno, int reason = 0); // output 1 packet
	virtual int need_send();    		// send ACK/win-update now?
	virtual void opencwnd();            // by zhanghan for l2dct
};


class LPDTcpAgent : public SackFullTcpAgent {

	virtual void slowdown(int how);			/* reduce cwnd/ssthresh */
	virtual int byterm();
	virtual int foutput(int seqno, int reason = 0); // output 1 packet
	virtual int need_send();    		// send ACK/win-update now?
    virtual void opencwnd();                // call when window increases
...
class DTcpAgent : public SackFullTcpAgent {
    ...
    virtual void delay_bind_init_all();
	virtual int delay_bind_dispatch(const char *varName, const char *localName, TclObject *tracer);
    int options_;   //0: deadline only 1: flowsize only 2: deadline+flowsize 

    // zhanghan for MLD, wmin,wmax,thredshold_min, threshold_max
	double deadline_thre_min_;
	double deadline_thre_max_;
	double flowsize_thre_min_;
	double flowsize_thre_max_;
	double flowsize_wmax_;
	double flowsize_wmin_;
	double deadline_wmax_;
	double deadline_wmin_;
...
//  --add by zhanghan this is Timely TCP agent
class TimelyTcpAgent: public SackFullTcpAgent {
...
public:
	...
	virtual void delay_bind_init_all();
	virtual int delay_bind_dispatch(const char *varName, const char *localName, TclObject *tracer);
        void update_rate(Packet* pkt);                   // calculate sending rate according to RTT
        int segment_ack;                                 // total acks receive of this segment
        int  threshold_;                                 // how many number of packets share the ack
        int  ackrecv;                                    // number of acks receive
	double   v_alpha_;    	// weight value of alpha
	double   v_sita_;
	double   v_beta_;  	    // rate weight value

	double rtt_diff;       // diff value of RTT
	double rtt_last;

	double  TLow;              // in ns, the low  bound  of RTT
	double THigh;            // in ns, the high bound  of RTT
        double v_rtt_;
        double minRTT;           //This is the min RTT

        double last;             // last compute time
        double timertt_;         
