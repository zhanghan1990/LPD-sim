...
static class LPDTcpClass : public TclClass {
public:
	LPDTcpClass() : TclClass("Agent/TCP/FullTcp/Sack/LPDTCP") {}
	TclObject* create(int, const char*const*) {
		return (new LPDTcpAgent());
	}
} class_lpd_full;



static class DTcpClass : public TclClass {
public:
	DTcpClass() : TclClass("Agent/TCP/FullTcp/Sack/DTCP") {}
	TclObject* create(int, const char*const*) {
...
static class TimelyTcpClass : public TclClass {
public:
	TimelyTcpClass() : TclClass("Agent/TCP/FullTcp/Sack/TimelyTCP") {}
	TclObject* create(int, const char*const*) {
		return (new TimelyTcpAgent());
	}
} class_timely_full;



static class LLTcpClass : public TclClass {
public:
	LLTcpClass() : TclClass("Agent/TCP/FullTcp/Sack/LLTCP") {}
	TclObject* create(int, const char*const*) {
		return (new LLTcpAgent());
	}
} class_ll_full;
...
FullTcpAgent::delay_bind_init_all()
{
	...
	delay_bind_init_one("flow_size_"); //zhanghan
	...
	delay_bind_init_one("wmin_");//zhanghan
	delay_bind_init_one("wmax_");//zhanghan
    delay_bind_init_one("D_MAX_");//zhanghan
    delay_bind_init_one("S_MAX_");//zhanghan
	delay_bind_init_one("threshold_min_");//zhanghan
	delay_bind_init_one("threshold_max_");//zhanghan
...
FullTcpAgent::delay_bind_dispatch(const char *varName, const char *localName, TclObject *tracer)
{
	...
	if (delay_bind(varName, localName, "flow_size_", &flow_size_, tracer)) return TCL_OK; // zhanghan
	...
	if (delay_bind(varName, localName, "wmin_", &wmin_, tracer)) return TCL_OK; // zhanghan for L2dct
	if (delay_bind(varName, localName, "wmax_", &wmax_, tracer)) return TCL_OK; // zhanghan for l2dct
	if (delay_bind(varName, localName, "threshold_min_", &threshold_min_, tracer)) return TCL_OK; // zhanghan for l2dct
	if (delay_bind(varName, localName, "threshold_max_", &threshold_max_, tracer)) return TCL_OK; // zhanghan for l2dct
	if (delay_bind(varName, localName, "D_MAX_", &D_MAX_, tracer)) return TCL_OK; // zhanghan for LPD
	if (delay_bind(varName, localName, "S_MAX_", &S_MAX_, tracer)) return TCL_OK; // zhanghan for LPD
	...
	if (argc == 3) {
		if (strcmp(argv[1], "advance") == 0) {
			advanceby(atoi(argv[2]));
                        // change to bytes
			flow_size_ = atoi(argv[2])*maxseg_;
			return (TCL_OK);
		}
...
FullTcpAgent::sendpacket(int seqno, int ackno, int pflags, int datalen, int reason, Packet *p)
{
	...
	tcph->hlen() += build_options(tcph);
        // add by zhanghan: for timely, write down the send time
       tcph->sendtime() = now();
	...
	*/
	...
	if (!nopredict_ && predict_ok(pkt)) {
		...
		if (datalen == 0) {
			...
			//printf("flow_remaining before dec = %d flowsent =%d\n" , flow_remaining_,flow_sent_);
		...
		//	printf("flow sent after sending %d\n",flow_sent_);
		//	printf("flow_remaining after dec = %d flowsent =%d\n" , flow_remaining_,flow_sent_);
...
/* dodata: this label is in the "real" code.. here only for reference */
	...
	if ((datalen > 0 || (tiflags & TH_FIN)) &&
		...
		if (tcph->seqno() == rcv_nxt_ && rq_.empty()) {
			...
			//printf("flow_remaining=%d flowsent before dec =%d\n" , flow_remaining_,flow_sent_);
				if (flow_remaining_ > 0) {
				      flow_remaining_ -= datalen; // Mohammad
			...
			//printf("flow sent after sending %d\n",flow_sent_);
			//printf("flow_remaining=%d flowsent after dec =%d\n" , flow_remaining_,flow_sent_);
...
LPDTcpAgent::slowdown(int how) {
	...
	double penalty = ecnhat_alpha_;
       //printf("deadline-aware: deadline = %d now  = %lf start_time=%lf tleft=%lf\n",deadline,now(),start_time,deadline/1e6 - (now() - start_time));
	
    // This is for deadline-aware flows
	if (deadline > 0) {
		double tleft = deadline/1e6 - (now() - start_time);              
		//if (tleft < 0 && now() < 3) {
		//	cwnd_ = 1;
		...
		//fflush(stdout);
	} else if(deadline ==0){// this is for flowsize-aware tcp, none-deadline-aware
		double flowsent = (double)(flow_size_)/1024.0;
		// LPD in KB
		//printf("flow size =%d sent = %lf SMAX= %lf byterm=%d\n",flow_size_,flowsent,S_MAX_,byterm());
		if(flowsent <=0)
			flowsent = S_MAX_;
		else if(flowsent >= S_MAX_)
			flowsent= S_MAX_;
		   penalty = ecnhat_alpha_*(flowsent)/S_MAX_;
	//	printf("flow-size aware penalty %lf S_MAX_ =%lf flowsent=%lf ,d=%lf\n",penalty,S_MAX_,flowsent,(flowsent)/S_MAX_);
	    }else if (penalty > 0) {
		//non-deadline->TCP
		penalty = 1;
	}
	...
	if (how & CLOSE_SSTHRESH_HALF)
		...
		ssthresh_ = (int)((1 - penalty) * windowd());
	...
	if (how & CLOSE_CWND_HALF)
		...
		if (first_decrease_ == 1 || slowstart || decrease_num_ == 0.5) {
			cwnd_ = halfwin;
                ...
                double before=windowd();
                cwnd_=windowd()*(1-penalty);
                double win__=cwnd_;
               // printf("old=%lf,penalty=%lf window=%lf\n",before,1-penalty,win__);
		if (cwnd_ < 1)
			cwnd_ = 1;
		}
...
void LPDTcpAgent::opencwnd()
{
       double penalty = ecnhat_alpha_;
       // zhanghan: LPD for deadline flows
	if (deadline != 0) {
		double tleft = deadline/1e6 - (now() - start_time);              
		if (tleft < 0) {
			tleft = 1e10;	
		...
		if(flowsent <=0)
		   ...
		   penalty = ecnhat_alpha_*(flowsent)/S_MAX_;
		//printf("flow-size aware penalty %lf\n",penalty);
	    }else if (penalty > 0) {
		//non-deadline->TCP
		penalty = 1;
	}

       double factor = 1-penalty;
	double increment;
      //printf("factor =%lf\n",factor);
	if (cwnd_ < ssthresh_) {
		...
		case 0:
			if (++count_ >= cwnd_) {
				count_ = 0;
				cwnd_ += factor;
                                //printf("cwnd_=%lf factor=%lf,alpha=%lf pe=%lf\n",windowd(),factor,ecnhat_alpha_,(double)flow_size_/1024.0/S_MAX_);
			}
			break;
...
int
LPDTcpAgent::foutput(int seqno, int reason) {
	if (deadline != 0) {
...
{
    double penalty = ecnhat_alpha_;
   // printf("opencwndm_: flowsize-aware: flowsize_ = %d wmin_  = %lf wmax_=%lf threshold_min_=%lf   threshold_max_=%lf\n",flow_size_,wmin_,wmax_,threshold_min_,threshold_max_);
	...
	//compute penalty 
     ...
     if(flowsent < threshold_min_){
     	wc = wmax_;
     } else if(flowsent > threshold_max_) {
     	wc = wmin_;
     } else {
     	wc = wmax_-(flowsent-threshold_min_)*(wmax_-wmin_)/(threshold_max_-threshold_min_);
     }
     penalty = pow(ecnhat_alpha_,wc);
    // printf("penalty=%lf,wc=%lf,alpha=%lf\n",penalty,wc,ecnhat_alpha_);

    double factor = wc/wmax_;
	double increment;
   // printf("factor =%lf\n",factor);
	if (cwnd_ < ssthresh_) {
		/* slow-start (exponential) */
...
LLTcpAgent::slowdown(int how) {
	...
	//compute penalty 
     ...
     penalty = pow(ecnhat_alpha_,wc);
     //printf("flowsent=%lf wc=%lf, penalty=%lf min =%lf max=%lf\n",flowsent,wc,penalty,threshold_min_,threshold_max_);
 ///    printf("penalty=%lf,wc=%lf,alpha=%lf\n",penalty,wc,ecnhat_alpha_);
	//ecnhat_alpha_ = 0.07;
	// we are in slowstart for sure if cwnd < ssthresh
	if (cwnd_ < ssthresh_) 
...
int
LLTcpAgent::foutput(int seqno, int reason) {
	if (deadline != 0) {
// 		double tleft = double(deadline)/1e6 - (now() - start_time) - byterm()*8/1e10;
		double tleft = deadline/1e6 - (now() - start_time) - (curseq_ - int(maxseq_)) * 8/1e10;
   		if (tleft < 0 && signal_on_empty_) {
...
void
TimelyTcpAgent::delay_bind_init_all()
{
	delay_bind_init_one("v_alpha_");
	delay_bind_init_one("v_beta_");
	delay_bind_init_one("v_sita_");
	delay_bind_init_one("v_low_");
	delay_bind_init_one("v_high_");
	delay_bind_init_one("v_rtt_");
        delay_bind_init_one("threshold_");
	SackFullTcpAgent::delay_bind_init_all();
}

int
TimelyTcpAgent::delay_bind_dispatch(const char *varName, const char *localName, 
				   TclObject *tracer)
{
	/* init vegas var */
		...
		return TCL_OK;
	if (delay_bind(varName, localName, "v_sita_", &v_sita_, tracer)) 
		return TCL_OK;
        if (delay_bind(varName, localName, "v_low_", &TLow, tracer)) 
		return TCL_OK;
	if (delay_bind(varName, localName, "v_high_", &THigh, tracer)) 
		return TCL_OK;
        if (delay_bind(varName, localName, "v_rtt_", &v_rtt_, tracer)) 
		return TCL_OK;
        if (delay_bind(varName, localName, "threshold_", &threshold_, tracer)) 
		return TCL_OK;
        return SackFullTcpAgent::delay_bind_dispatch(varName, localName, tracer);
}

TimelyTcpAgent::TimelyTcpAgent(){
     rtt_diff = 0;
     rtt_last = 0;
     minRTT = 99999999;
     ackrecv = 0 ;
...
void 
TimelyTcpAgent::update_rate(Packet* pkt)
{
      //get RTT(in second)
      hdr_tcp *tcph = hdr_tcp::access(pkt);	// TCP header
      timertt_ += (now()-tcph->sendtime());

      ackrecv++;
       if(ackrecv < threshold_)
          return;
      ackrecv = 0;
      timertt_ /=(double)threshold_;
    //  printf("timertt= %lf\n",timertt_);

      //renew the value of min RTT;
      minRTT = minRTT < timertt_? minRTT:timertt_;
     // printf("now = %lf fid = %d minRTT = %lf timertt_=%lf\n",now(),fid_,minRTT,timertt_);
      double new_rtt_diff = timertt_-rtt_last;
      rtt_last = timertt_;

      //compute the new weight add rtt difference.
      rtt_diff = (1-v_alpha_)*rtt_diff+v_alpha_*new_rtt_diff;
      //printf("after::rtt_diff = %lf timertt= %lf minRTT= %lf low =%lf  high= %d \n",rtt_diff,timertt_,minRTT,TLow,THigh);
 
     //get the gradient
      double normalized_gradient = 0;
      if(minRTT !=0)
         normalized_gradient = rtt_diff/minRTT;

       double time_ = now() - last;
       last = now();
       double rate = threshold_*maxseg_/time_/1000/1000*8;
       double rttus = timertt_ *1000000;
       double r1 = rate;
       ...
       if(rttus < TLow) {
           ...
           rate = rate* (1-v_beta_*(1-(double)THigh/rttus));
       }
       else if(normalized_gradient<=0) {
           rate = rate +  v_sita_;
       } else {
	   rate = rate*(1-v_beta_*normalized_gradient);
       }
     
      
     //adjust the congestion window now
      double expect = rate*1000*1000/8/maxseg_*timertt_;
      double con  = cwnd_;
      //printf("flow %d r1 = %lf rate = %lf expect =%lf cwnd= %lf\n",fid_,r1,rate,expect,con);
      cwnd_ = expect;
     // printf("expect window size%lf  cwnd= %lf\n",expect,con);
}

void
TimelyTcpAgent::recv(Packet *pkt, Handler*)
{
	//Shuang: cancel probe mode
		prob_mode_ = false;
		prob_count_ = 0;
    ...
    if(options_==0)
     {
        double wc;
       // change flow_size_ to KB;
       double durtime = ((double)(now()-start_time));  //in seconds
       if(durtime < deadline_thre_min_){
     	  wc = deadline_wmin_;
        } else if(durtime > deadline_thre_max_) {
     	  wc = deadline_wmax_;
        } else {
     	  wc = deadline_wmin_+(durtime-deadline_thre_min_)*(deadline_wmax_-deadline_wmin_)/(deadline_thre_max_-deadline_thre_min_);
        }
       penalty=ecnhat_alpha_*wc;
     }
     // only flowsize
     else if(options_==1)
     {
       ...
       double flowsent = ((double)(flow_size_-byterm()))/1024.0;
       double remainsize = (double)(byterm()/1024.0);
       //printf("DTCP down flow size =%lf sent = %lf  remainsize=%lf\n",flow_size_/1024,flowsent,remainsize);
       if(flowsent < flowsize_thre_min_){
     	  wc = flowsize_wmin_;
        } else if(flowsent > flowsize_thre_max_) {
     	  wc = flowsize_wmax_;
        } else {
     	  wc = flowsize_wmin_+(flowsent-flowsize_thre_min_)*(flowsize_wmax_-flowsize_wmin_)/(flowsize_thre_max_-flowsize_thre_min_);
        }
       penalty=ecnhat_alpha_*wc;
     ...
     else
     {
       ...
       if(durtime < deadline_thre_min_){
     	  ...
     	  wc1 = deadline_wmin_+(durtime-deadline_thre_min_)*(deadline_wmax_-deadline_wmin_)/(deadline_thre_max_-deadline_thre_min_);
        }
       if(flowsent < flowsize_thre_min_){
     	  wc2 = flowsize_wmin_;
        } else if(flowsent > flowsize_thre_max_) {
     	  ...
     	  wc2 = flowsize_wmin_+(flowsent-flowsize_thre_min_)*(flowsize_wmax_-flowsize_wmin_)/(flowsize_thre_max_-flowsize_thre_min_);
        }

       penalty=ecnhat_alpha_*wc1*wc2;
       //printf("penalty=%lf,wc1=%lf,wc2=%lf,alpha=%lf\n",penalty,wc1,wc2,ecnhat_alpha_);
...
int
DTcpAgent::foutput(int seqno, int reason) {
	if (deadline != 0) {
// 		double tleft = double(deadline)/1e6 - (now() - start_time) - byterm()*8/1e10;
		double tleft = deadline/1e6 - (now() - start_time) - (curseq_ - int(maxseq_)) * 8/1e10;
   		if (tleft < 0 && signal_on_empty_) {
...
void
DTcpAgent::delay_bind_init_all()
{

	delay_bind_init_one("deadline_thre_min_");
	delay_bind_init_one("deadline_thre_max_");
	delay_bind_init_one("flowsize_thre_min_");
	delay_bind_init_one("flowsize_thre_max_");
	delay_bind_init_one("flowsize_wmax_");
	delay_bind_init_one("flowsize_wmin_");
	delay_bind_init_one("deadline_wmax_");
	delay_bind_init_one("deadline_wmin_");
	delay_bind_init_one("options_");
	SackFullTcpAgent::delay_bind_init_all();
}

int
DTcpAgent::delay_bind_dispatch(const char *varName, const char *localName, 
				   TclObject *tracer)
{
        if (delay_bind(varName, localName, "options_", &options_, tracer)) 
        	return TCL_OK;
        if (delay_bind(varName, localName, "deadline_thre_min_", &deadline_thre_min_, tracer)) 
        	return TCL_OK;
        if (delay_bind(varName, localName, "deadline_thre_max_", &deadline_thre_max_, tracer)) 
        	return TCL_OK;
        if (delay_bind(varName, localName, "flowsize_thre_min_", &flowsize_thre_min_, tracer)) 
        	return TCL_OK;
        if (delay_bind(varName, localName, "flowsize_thre_max_", &flowsize_thre_max_, tracer)) 
        	return TCL_OK;
        if (delay_bind(varName, localName, "flowsize_wmax_", &flowsize_wmax_, tracer)) 
        	return TCL_OK;
        if (delay_bind(varName, localName, "flowsize_wmin_", &flowsize_wmin_, tracer)) 
        	return TCL_OK;
        if (delay_bind(varName, localName, "deadline_wmax_", &deadline_wmax_, tracer)) 
        	return TCL_OK;
        if (delay_bind(varName, localName, "deadline_wmin_", &deadline_wmin_, tracer)) 
        	return TCL_OK;
        return SackFullTcpAgent::delay_bind_dispatch(varName, localName, tracer);
}
