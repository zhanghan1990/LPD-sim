...
if {$getQmonLinkLevelStats == 1} {
	# Queue Mon for our topology, for link between Agg and Core 
	for { set i 0 } { $i < $num_cores } { incr i } {
		for { set j 0 } { $j < $num_aggs } { incr j } {
			set aggIndex [expr ($i*$num_aggs)+$j] 
			...
			$ns at [expr $STATS_START+$STATS_INTR] "linkDump [$ns link $agg($aggIndex) $core($i)] $bing_ab($aggIndex) $ping_ab($aggIndex) $qmon_ab($aggIndex) $STATS_INTR A-B $fq_mon($aggIndex) $f_util($aggIndex) $f_loss($aggIndex) $f_queue($aggIndex) [expr $qLimit*$pkSize]" ; 
		}
	}
}
